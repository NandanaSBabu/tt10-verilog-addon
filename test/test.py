...
def test_project(dut):
    ...
    # Reset the DUT
    dut.rst_n <= 0
    yield cocotb.regression.Timer(10, units='ns')  # Apply reset for 10ns
    dut.rst_n <= 1
    yield cocotb.regression.Timer(10, units='ns')  # Wait for 10ns to deassert reset

    # Enable DUT and apply inputs
    dut.ena <= 1
    dut.ui_in <= ui_in
    dut.uio_in <= uio_in
    yield cocotb.regression.Timer(20, units='ns')  # Wait for 20ns for output to stabilize
    ...
    dut.ui_in <= ui_in
    dut.uio_in <= uio_in
    yield cocotb.regression.Timer(20, units='ns')  # Wait for 20ns
    ...
    # End simulation
    yield cocotb.regression.Timer(100, units='ns')
